...
template <class C>
struct RangeSearchResultHandler { // 可以处理多个查询（单线程）
    using T = typename C::T;
    using TI = typename C::TI;
    ...
    struct SingleResultHandler {
        ...
        float radius;
        RangeQueryResult* qr = nullptr; // 存储1个查询向量的所有查询结果
        ...
        void add_result(T dis, TI idx) {
            if (C::cmp(radius, dis)) {
                qr->add(dis, idx); // 直接放到 buffer 的最后一个位置
            }
        }
    ...
    std::vector<RangeSearchPartialResult*> partial_results; // 用于 blas 版存储
    ...
    void add_results(size_t j0, size_t j1, const T* dis_tab) { // 用于 blas
        RangeSearchPartialResult* pres;
...
struct SingleBestResultHandler {
    ...
    struct SingleResultHandler {
        ...
        /// add one result for query i
        void add_result(T dis, TI idx) {
            if (C::cmp(min_dis, dis)) { // min_dis > dis
                min_dis = dis;
                min_idx = idx;
            }
        }
