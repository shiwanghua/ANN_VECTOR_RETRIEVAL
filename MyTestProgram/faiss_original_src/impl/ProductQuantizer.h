...
struct ProductQuantizer : Quantizer {
    ...
    float* get_centroids(size_t m, size_t i) { // 第 m 堆短向量的第 i 个质心
    ...
    void set_derived_values(); // 算 dsub、ksub

    /// Define the centroids for subquantizer m 给第 m 堆的 ksub 个质心赋值
    ...
    /// more efficient to compute the codes. // 只编码了1个向量
    ...
    std::vector<float> sdc_table; // 堆->质心->质心->距离 [M,ksub,ksub]
...
struct PQEncoderGeneric { // 通用，表示存储的 nbits 位可通过构造函数改变，nbits=8或16就相当于下面的PQEncoder8、PQEncoder16
