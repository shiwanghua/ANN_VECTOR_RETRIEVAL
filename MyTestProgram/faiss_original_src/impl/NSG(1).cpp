...
struct NegativeDistanceComputer : DistanceComputer { // 对IP距离(相似度)取负号，统一成距离越小越好
...
inline int insert_into_pool(Neighbor* addr, int K, Neighbor nn) { // 二分插入一个按距离增序的数组里，已有 K 个元素，空间已预先分配好，返回 0~K 表示按序插入 nn 的位置，返回 K+1 表示已经有 nn 了 
    ...
    if (addr[right].distance < nn.distance) { // 调用之前已判断，不可能进入 if
        addr[K] = nn;
        return K;
    }
    while (left < right - 1) {
...
void NSG::search(
    ...
    std::vector<Neighbor> retset; // store the query result
    std::vector<Node> tmp;
...
#pragma omp parallel for // 计算每个节点的实际度数
        for (int i = 0; i < n; i++) {
            int cnt = 0;
            for (int j = 0; j < R; j++) {
...
void NSG::init_graph(Index* storage, const nsg::Graph<idx_t>& knn_graph) {
    ...
    std::unique_ptr<float[]> center(new float[d]); // 平均值向量
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < d; j++) {
            center[j] += tmp[j]; // 取出每个向量，对每一维求和
        }
    }

    for (int i = 0; i < d; i++) { // 求每一维的平均值
        center[i] /= n;
    }
    ...
    int ep = rng.rand_int(n); // enterpoint, 从该节点出发
...
void NSG::search_on_graph(
    ...
    std::vector<int> init_ids(pool_size); // max(16,k)
    ...
    for (int i = 0; i < init_ids.size() && i < graph.K; i++) {
        ...
        init_ids[i] = id; // 把 ep 的邻居作为初始值
        vt.set(id);
        num_ids += 1;
    }

    while (num_ids < pool_size) { // 随机取满 pool_size 个节点
    ...
    std::sort(retset.begin(), retset.begin() + pool_size); // 按距离排序

    int k = 0;
    while (k < pool_size) { // k = pool_size 了，就说明数组里所有节点及其 BFS 能遍历到的更近的节点都遍历完了
        ...
        if (retset[k].flag) {
            ...
            for (int m = 0; m < graph.K; m++) { // BFS 往外扩充
                ...
                // 插入更近的节点
                int r = insert_into_pool(retset.data(), pool_size, nn); // 返回插入位置

                updated_pos = std::min(updated_pos, r); // 下标越小则越近
            }
        }
        // 从 k 号节点的邻居里找到了更近的节点下标 updated_pos，则下次处理这个最近节点，否则处理 k+1 下标的节点
