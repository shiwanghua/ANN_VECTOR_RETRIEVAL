...
struct HNSW {
    ...
    std::vector<int> cum_nneighbor_per_level; // 每层节点累计和
    ...
    /// size ntotal + 1
    std::vector<size_t> offsets; // 是那层的第几个元素-1 ？
    ...
    void setEFConstruction(int);

    void setEFSearch(int);
...
struct HNSWStats {
    size_t n1, n2, n3; // n1 search_from_candidates 处理第0层的次数 + search_from_candidate_unbounded() 调用次数，n2 candidates 处理完的次数，n3 bfs 扩展了多少个节点入堆
    size_t ndis;
    size_t nreorder;
