...
void HNSW::setEFConstruction(int efc) {
    efConstruction = efc;
}

void HNSW::setEFSearch(int efs) {
    efSearch = efs;
}
...
void HNSW::set_nb_neighbors(int level_no, int n) {
    ...
    for (int i = level_no + 1; i < cum_nneighbor_per_level.size();
         i++) { // +1 means the cum of the first level_no layers
...
void HNSW::neighbor_range(idx_t no, int layer_no, size_t* begin, size_t* end)
    ...
    *end = o + cum_nb_neighbors(layer_no + 1); // end-begin==M or 2M
}
...
HNSW::HNSW(int M) : rng(12345) {
    ...
    entry_point = -1;
    efSearch = 100;       // 16;
    efConstruction = 100; // 40;
    upper_beam = 1;
    offsets.push_back(0);
}

int HNSW::random_level() { // return 0 for 90%+ conditions
...
void HNSW::set_default_probas(
        int M,
        float levelMult) { // M=10: 9levels; M=64: 5levels
    int nn = 0;
    cum_nneighbor_per_level.push_back(0); // nlevels+1 because cummulative
    for (int level = 0;; level++) {
        float proba = exp(-level / levelMult) *
                (1 - exp(-1 / levelMult)); // e{-level*log(M)} * (1-e{-log(M)})
        if (proba < 1e-9)
            break;
        assign_probas.push_back(
                proba); // begin from the second value, very small
        nn += level == 0 ? M * 2 : M;
        cum_nneighbor_per_level.push_back(nn); // 0, 2M, 3M, ...
    }
}

void HNSW::clear_neighbor_tables(int level) {
    for (int i = 0; i < levels.size(); i++) { // i is id of each vector?
        size_t begin, end;
...
#pragma omp parallel for reduction(+: tot_neigh) reduction(+: tot_common) \
  reduction(+: tot_reciprocal) reduction(+: n_node)
    for (int i = 0; i < levels.size(); i++) {
        if (levels[i] > level) { // vector i covers 1~level~levels[i]
            n_node++;
            size_t begin, end;
            neighbor_range(
                    i,
                    level,
                    &begin,
                    &end); // neighbors of vector i at $level layer
            ...
            for (size_t j = begin; j < end; j++) {
                ...
                for (size_t j2 = begin2; j2 < end2; j2++) {
                    ...
                    if (neighset.count(i3)) { // i->i2, i->i3, i2->i3
                        neighset.erase(i3);
                        n_common++;
                    }
                }
...
void HNSW::fill_with_random_links(size_t n) {
    ...
    for (int level = max_level - 1; level >= 0; --level) {
        std::vector<int> elts; // vector id that cover $level layer
        for (int i = 0; i < n; i++) {
            if (levels[i] > level) {
                elts.push_back(i);
            }
        ...
        for (int ii = 0; ii < elts.size(); ii++) {
            ...
            size_t begin, end;
            neighbor_range(
                    i, 0, &begin, &end); // 2M neighbors for the fisrt level
            for (size_t j = begin; j < end; j++) {
                int other = 0;
                do {
                ...
                neighbors[j] = other; // link i ($level layer) to random other
                                      // ($level layer) at 0 level
            }
        }
    }
}
...
int HNSW::prepare_level_tab(size_t n, bool preset_levels) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        offsets.push_back(
                offsets.back() +
                cum_nb_neighbors(pt_level + 1)); // 最下面 levels[i + n0] 个层一共多少个邻居
...
/** Enumerate vertices from farthest to nearest from query, keep a // ？为什么是从远到近，下面先pop的是最近的吧
 * neighbor only if there is no previous neighbor that is closer to
 * that vertex than the query.
 */
void HNSW::shrink_neighbor_list(
        DistanceComputer& qdis,
        std::priority_queue<NodeDistFarther>& input, // 默认是大根堆，但对于NodeDistFarther是其距离越小越在堆顶
        std::vector<NodeDistFarther>& output,
        int max_size) { // 从 input 里找 output 个，相互之间的距离大于等于自己到查询q的距离，即保留一些比较远的点
    while (input.size() > 0) {
        NodeDistFarther v1 = input.top(); // 先返回距离最小的（最下层的？但后面最开始给第一层赋值是从最远开始的）
        input.pop();
        float dist_v1_q = v1.d;

        bool good = true;
        for (NodeDistFarther v2 : output) { // v1,v2,v3,...vj,...   q     
            float dist_v1_v2 = qdis.symmetric_dis(v2.id, v1.id);

            if (dist_v1_v2 < dist_v1_q) {  // 前 j-1 个到 vj 的对称距离大于等于 查询 q 到 vj 的距离才保留
                good = false;
                break;
            }
        }
...
void shrink_neighbor_list(
        DistanceComputer& qdis,
        std::priority_queue<NodeDistCloser>& resultSet1, // 大根堆，堆顶的距离最大
        int max_size) {
    if (resultSet1.size() < max_size) {
        return;
    }
    std::priority_queue<NodeDistFarther> resultSet; // 相当于距离小根堆
    std::vector<NodeDistFarther> returnlist;

    while (resultSet1.size() > 0) { // 大根堆转小根堆
    ...
    for (NodeDistFarther curen2 : returnlist) { // 结果又存到大根堆里
...
void add_link(
    ...
    if (hnsw.neighbors[end - 1] == -1) {
        ...
        size_t i = end;
        while (i > begin) { // 找到最小的空位置（最低的层）
            if (hnsw.neighbors[i - 1] != -1)
                break;
            i--;
        }
        hnsw.neighbors[i] = dest; // 单项边
        return;
    }
    ...
    // copy to resultSet...
    std::priority_queue<NodeDistCloser> resultSet; // 大根堆，越上层的节点越在堆顶
    resultSet.emplace(qdis.symmetric_dis(src, dest), dest); // 把 src 当作查询向量了
    for (size_t i = begin; i < end; i++) { // HERE WAS THE BUG
        storage_idx_t neigh = hnsw.neighbors[i];
        resultSet.emplace(qdis.symmetric_dis(src, neigh), neigh);
    }

    shrink_neighbor_list(qdis, resultSet, end - begin); // 一共end-begin+1个，至少会减少1个

    // ...and back
    size_t i = begin;
    while (resultSet.size()) {
        hnsw.neighbors[i++] = resultSet.top().id; // 对于level层的邻居，先放距离最远的向量！
        resultSet.pop();
    }
...
void search_neighbors_to_add(
    ...
    while (!candidates.empty()) {
        ...
        // loop over neighbors
        size_t begin, end;
        hnsw.neighbor_range(currNode, level, &begin, &end); // 遍历最近的节点在level层的每个未访问邻居，保留和深入探索这些更近的邻居向量
        for (size_t i = begin; i < end; i++) {
            ...
            // 在level层用 BFS 找尽可能近的 efConstruction 个向量
            if (results.size() < hnsw.efConstruction || results.top().d > dis) {
                results.emplace(dis, nodeId); // 可能是 efConstruction+1 个了
...
void greedy_update_nearest(
        ...
        float& d_nearest) { // 在 level 层的邻居中找最近的1个向量，如果有更近的，bfs 继续找更近的，否则返回，搜索宽度是1
    for (;;) {
...
void HNSW::add_links_starting_from(
    ...
    faiss::shrink_neighbor_list(ptdis, link_targets, M); // 只保留 M 个（不一定是离 entry_point 最近的）

    std::vector<storage_idx_t> neighbors;
    neighbors.reserve(link_targets.size());
    while (!link_targets.empty()) {
        storage_idx_t other_id = link_targets.top().id;
        add_link(*this, ptdis, pt_id, other_id, level); // 单向边
    ...
    for (storage_idx_t other_id : neighbors) {
        ...
        add_link(*this, ptdis, other_id, pt_id, level); // 反向边
...
#pragma omp critical
    ...
    for (; level > pt_level; level--) { // 找上面几层中最近的向量
        greedy_update_nearest(*this, ptdis, level, nearest, d_nearest);
    }

    for (; level >= 0; level--) { // 0~level 层做插入、连边操作
    ...
    if (pt_level > max_level) { // entry_point 必须是最上层的向量
        max_level = pt_level;
        entry_point = pt_id;
    }
}
...
int HNSW::search_from_candidates(
    ...
    while (candidates.size() > 0) {
        ...
        nstep++;
        if (!do_dis_check && nstep > efSearch) { // bfs 最多做efSearch次
            break;
        }
    }
...
std::priority_queue<HNSW::Node> HNSW::search_from_candidate_unbounded(
    ...
    int ndis = 0;
    std::priority_queue<Node> top_candidates; // 大根堆
    std::priority_queue<Node, std::vector<Node>, std::greater<Node>> candidates; // 小根堆
    ...
    while (!candidates.empty()) {
        ...
        if (d0 > top_candidates.top().first) { // 没有更近的了
            break;
        }
