...
class CostCalculator {
   ...
   public:
    CostCalculator(const std::vector<float>& vec, idx_t n) { // O(n)
    ...
    float operator()(idx_t i, idx_t j) { // O(1)
        if (j < i) {
            return 0.0f;
        }
        auto mu = (cumsum[j + 1] - cumsum[i]) / (j - i + 1); // 取平均值作为质心
        auto result = cumsum2[j + 1] - cumsum2[i]; // 平方和
        result += (j - i + 1) * (mu * mu);
        result -= (2 * mu) * (cumsum[j + 1] - cumsum[i]); // 即距离
        return float(result);
    }
};
    ...
    ****************************************************/
    ...
    for (idx_t k = 1; k < nclusters; ++k) {
        ...
        auto C = [&D, &CC, &k](idx_t m, idx_t i) {
            ...
            idx_t col = std::min(m, i - 1); // i 有可能大于 m+1 ？
