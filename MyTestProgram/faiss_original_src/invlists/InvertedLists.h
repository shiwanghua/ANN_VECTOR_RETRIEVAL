 ...
 *   are allowed
 */
struct InvertedLists { // 没有存储id和向量code
    ...
    virtual void release_codes(size_t list_no, const uint8_t* codes) const; // 释放空间，为什么是 Read only ？
    ...
    virtual ~InvertedLists(); // 父结构体的析构函数必须是虚函数
    ...
    struct ScopedCodes {
        ...
        ScopedCodes(const InvertedLists* il, size_t list_no) // 一个倒排表
                : il(il), codes(il->get_codes(list_no)), list_no(list_no) {}

        ScopedCodes(const InvertedLists* il, size_t list_no, size_t offset) // 倒排表里某条记录开始的列表
                : il(il),
        ...
        ~ScopedCodes() {
            il->release_codes(list_no, codes); // 只能删除这个倒排表上的一部分向量
        }
    };
};
...
struct ArrayInvertedLists : InvertedLists {
    std::vector<std::vector<uint8_t>> codes; // binary codes, size nlist // 不是存储的指针，所以不需要重写 release_codes
...
struct ReadOnlyInvertedLists : InvertedLists { // 没有重写 list_size 等函数，还是个纯虚结构体，不能被实例化
...
struct HStackInvertedLists : ReadOnlyInvertedLists { // 每个 ReadOnlyInvertedLists 的子类重写 8 个函数
...
struct VStackInvertedLists : ReadOnlyInvertedLists {
    std::vector<const InvertedLists*> ils;
    std::vector<idx_t> cumsz; // 记录倒排表质心个数数组的前缀和
