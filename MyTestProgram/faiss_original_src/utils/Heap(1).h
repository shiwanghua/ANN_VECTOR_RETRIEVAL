...
inline void heap_replace_top(
    ...
    while (1) {
        ...
        if ((i2 == k + 1) ||
            C::cmp2(bh_val[i1], bh_val[i2], bh_ids[i1], bh_ids[i2])) { // 没有右孩子了或者左孩子比较大
            if (C::cmp2(val, bh_val[i1], id, bh_ids[i1])) { // 被插入元素 val 已经是最大的了不需要往下降了
                break;
            }
            bh_val[i] = bh_val[i1]; // 左孩子比较大，提上来
            bh_ids[i] = bh_ids[i1];
            i = i1;
        } else {
            if (C::cmp2(val, bh_val[i2], id, bh_ids[i2])) {
...
template <typename C>
inline size_t heap_reorder( // 从堆构建有序数组，O(N)
        size_t k,
        typename C::T* bh_val,
        typename C::TI* bh_ids) {
    size_t i, ii;
