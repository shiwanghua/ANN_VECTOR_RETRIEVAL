...
struct ClusteringParameters {
    ...
    bool update_index;     ///< re-train index after each iteration?  是否每次迭代都用调用一次索引里的训练函数
    bool frozen_centroids; ///< use the centroids provided as input and do not
                           ///< change them during iterations // 有几个质心不能修改的意思

    // 指每个类最少、最多包含多少个点
    int min_points_per_centroid; ///< otherwise you get a warning
    int max_points_per_centroid; ///< to limit size of dataset // 实际聚类时并没有判断是不是超过了最大值，这只是个期望值，用于限制所有类的向量数之和的
...
struct ClusteringIterationStats {
    ...
    double time; ///< seconds for iteration 递增的总训练时间
...
struct Clustering : ClusteringParameters {
    ...
    size_t k; ///< nb of centroids // nlist

    /** centroids (k * d)
     * if centroids are set on input to train, they will be used as // 下面的构造函数里没有输入float数组作为初始质心
     * initialization
     */
...
struct ProgressiveDimClusteringParameters : ClusteringParameters {
    int progressive_dim_steps; ///< number of incremental steps 步数，先后对前 1、2、4、8、...、d 个维度进行聚类
...
struct ProgressiveDimIndexFactory { // 构造不同维度的索引
 ...
 */
struct ProgressiveDimClustering : ProgressiveDimClusteringParameters { // 数据成员和 Clustering 一样
 ...
 * @param k nb of output centroids // nlist
 * @param x training set (size n * d)
 * @param centroids output centroids (size k * d)
 * @return final quantization error
 */
float kmeans_clustering( // IndexFlatL2 版
        size_t d,
        size_t n,
        size_t k,
        const float* x,
