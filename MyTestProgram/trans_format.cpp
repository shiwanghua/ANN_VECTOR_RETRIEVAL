...
void writeInputDoc( // generate doc input dataset of 
	...
	const float* xb)
{
	...
	for (size_t i = 0; i < nb; i++)
	{
		...
		if (i && i % 100000 == 0)
		{
			printf("generate %ld input doc", i);
			printSystemTime();
		}
	}
	...
	const char* groundTruthPath)
{
	...
	printf("read base dataset: nb= %ld, d= %ld, [%.2fs].\n",
		nb,
		d,
		elapsed() - t0);
	printSystemTime();
...
// 已有查询向量文件，直接读，生成答案文件
	t0 = elapsed();
	float* xq = fvecs_read_txt(queryFilePath, &d, &nquery);
	printf("read query dataset: nb= %ld, d= %ld, nq= %ld, [%.2fs].\n",
		nb, d, nquery, elapsed() - t0);
	printSystemTime();
	...
	for (int64_t i = 0; i < nquery; i += n_seg)
	{
		...
		for (int64_t j = 0; j < n_search; j++)
		{
			...
			{
				// std::cout << "i= " << i << ", j= " << j << ", u= " << u << ", " << std::to_string(gt[j * k + u])
				// 		  << "\n";
		...
		memset(gt, -1, n_seg * k);
		printf("%ld~%ld vector ground truth data is stored [%.2fs].\n",
			i,
			i + n_search,
			elapsed() - t0);
		printSystemTime();
...
int main()
{
	...
	// int64_t d64, nb64;
	// xb = fvecs_read_txt(
	//         "/home/shiwanghua/faiss-main-/demos/Data/china/txt/ID_embedding_data_full_base.txt",
	//         &d64,
	//         &nb64);
	// d = d64, nb = nb64;
	// printSystemTime();
	// writeInputDoc(
	//         "/home/shiwanghua/faiss-main-/demos/Data/china/_input/ID_embedding_data_full_base.input",
	//         d,
	//         nb,
	//         xb);

	// process_csv(
	//         "./Data/china_spark_sql_embeddings.csv",
	//         "./Data/china_spark_sql_embeddings_base.txt",
	//         10000,
	//         "./Data/china_spark_sql_embeddings_query.txt");

	// sampleTrainQueryGtDatasets(
	//         "/home/shiwanghua/faiss-main-/demos/Data/china/txt/ID_embedding_data_full_base.txt",
	//         508679,
	//         "/home/shiwanghua/faiss-main-/demos/Data/china/txt/ID_embedding_data_full_train.txt",
	//         10000,
	//         "/home/shiwanghua/faiss-main-/demos/Data/china/txt/ID_embedding_data_full_noise_query_k100.txt",
	//         100,
	//         "/home/shiwanghua/faiss-main-/demos/Data/china/txt/ID_embedding_data_full_noise_groundtruth_k100.txt");
